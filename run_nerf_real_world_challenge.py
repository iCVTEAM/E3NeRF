...
from load_blender import load_blender_data_exp_1_3D_adp
from load_blender import load_blender_data_exp_1_adp_8
from load_blender import pose_interpolation
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        '''
        rgbs_extra = extra["rgb0"]
        rgbs_extras.append(rgbs_extra.cpu().numpy())
        '''
        if i==0:
        ...
        if savedir is not None:
            ...
            '''
            rgb8 = to8b(rgbs_extras[-1])
            filename = os.path.join(savedir, '{:03d}extras.png'.format(i))
            imageio.imwrite(filename, rgb8)
            '''
...
def create_nerf(args):
    ...
    if args.dataset_type != 'ellff' or args.no_ndc:
        print('Not ndc!')
...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        if args.bin_num_evaluater:
            images, poses_4, poses_8, poses_12, render_poses, test_poses, hwf = load_blender_data_exp_1_adp_8(args.datadir)
            ...
            images, poses, render_poses, test_poses, hwf  = load_blender_data_exp_1_3D_adp(args.datadir, args.bin_num)
        ...
        new_poses = []
        test_poses = poses[8 : images.shape[0] * 17 : 17]
        test_poses_novel = poses[images.shape[0] * 17 : ]
        poses = poses[ : images.shape[0] * 17].reshape(16, 17, 3, 4)
        render_poses = pose_interpolation_llff(test_poses_novel, 5)

        print('Loaded llff', images.shape, render_poses.shape, hwf, args.datadir)

        # Motion-Guided Event Splitting
        if args.bin_num_evaluater:
    ...
    if use_event:
        if args.bin_num_evaluater:
            event_map_4 = torch.load(os.path.join(args.datadir, "events_4.pt"))
            event_map_8 = torch.load(os.path.join(args.datadir, "events_8.pt"))
            event_map_12 = torch.load(os.path.join(args.datadir, "events_12.pt"))
            frames_weights_4 = np.loadtxt(os.path.join(args.datadir, "frames_weights_4.npy"))
            frames_weights_8 = np.loadtxt(os.path.join(args.datadir, "frames_weights_8.npy"))
            frames_weights_12 = np.loadtxt(os.path.join(args.datadir, "frames_weights_12.npy"))
            event_maps = [event_map_4, event_map_8, event_map_12]
            frames_weights = [frames_weights_4, frames_weights_8, frames_weights_12]
        else:
            # load event
            event_maps = torch.load(os.path.join(args.datadir, "events.pt"))
            # load frame weights
            frames_weights = np.loadtxt(os.path.join(args.datadir, "frames_weights.npy"))
        # load masks
        event_mask = np.load(os.path.join(args.datadir, "event_mask.npy"))
        # rgb2grey tensor
    ...
    # Motion-Guided Event Splitting
    if args.bin_num_evaluater:
        bin_num_evaluater = bin_num_eval(images.shape[0], os.path.join(basedir, expname))
    ...
    for i in trange(start, N_iters):
        ...
        if len(blur_inds) != 0:
            ...
            blur_rgb = [rgb.view((bin_num + 1), -1, 3)[j] * frames_weight[j] for j in range(bin_num + 1)]

            blur_extras = [extras['rgb0'].view((bin_num + 1), -1, 3)[j] for j in range(bin_num + 1)]
            blur_depthes = [depth.detach().clone().view((bin_num + 1), -1, 1)[j] for j in range(bin_num + 1)]
            ...
            event_loss = event_loss_call_real(blur_rgbs, event_data, rgb2grey, bin_num) * 0.005

            img_loss_blur = img2mse(blur_rgb, target_blur)

            if 'rgb0' in extras:
                blur_extras = torch.stack(blur_extras, dim=0)
                img_loss0_blur = img2mse(blur_extras[int(bin_num/2)], target_blur)
        ...
        if len(sharp_inds) != 0:
            ...
            if 'rgb0' in extras:
                img_loss0_sharp = img2mse(extras['rgb0'], target_sharp)
        ...
        if i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(torch.tensor(render_poses), hwf, K, args.chunk, render_kwargs_test)
        ...
        if i%args.i_val==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs = render_path(torch.Tensor(val_poses).to(device), hwf, K, args.chunk, render_kwargs_test)
                # psnr = mse2psnr(img2mse(rgbs[0], gt[0]))
            filename = os.path.join(valsavedir, '{:03d}.png'.format(i))
            imageio.imwrite(filename, to8b(rgbs[0])[0])
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                render_path(torch.Tensor(np.array(test_poses)).to(device), hwf, K, args.chunk, render_kwargs_test, savedir=testsavedir)
                render_path(torch.Tensor(np.array(test_poses_novel)).to(device), hwf, K, args.chunk, render_kwargs_test, savedir=testsavedir + "_novel")
        ...
        if i%args.i_print==0:
            ...
            tqdm.write(f"[TRAIN] Iter: {i} Event_Loss: {event_loss.item()}  img_loss_blur: {img_loss_blur.item()} img_loss_sharp:  {img_loss_sharp.item()} PSNR: {psnr.item()}")
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  img_loss0_blur: {img_loss0_blur.item()}  img_loss0_sharp: {img_loss0_sharp.item()}")
            tqdm.write(f"[TRAIN] Iter: {i} Bin_num: {bin_num} Img_i: {img_i}")
