...
os.makedirs(basedir + "gt_8/", exist_ok=True)
...
for i in range(0, 200):
    ...
    if i % 2 == 0:
        ...
        for j in range(1, 20):
            ...
            if j == 9:
                cv2.imwrite(basedir + "gt_8/{0:03d}.png".format(int(i / 2)), img)

    else:
        img = cv2.imread(input_path + "1.png", cv2.IMREAD_UNCHANGED)
        img = rgba2rgb(img)
        img = cv2.cvtColor(img, cv2.COLOR_RGBA2RGB)
...
for i in range(0, 200, 2):
    ...
    command = "python  ./utils/blur_synthesize/main.py --input_dir {0}/original_rgb/r_{1}/".format(basedir, i) + \
              ...
              " --scale_factor 17 --input_exposure 10 --input_iso 50 --output_iso 50"
    try:
