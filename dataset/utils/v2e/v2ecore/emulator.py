...
class EventEmulator(object):
    ...
    def _init(self, first_frame_linear):
        ...
        if self.sigma_thres > 0:
            ...
            #change for torch 1.1.0 and below
            #qys = np.full([260,346], self.sigma_thres)
            qys = np.full([self.output_height, self.output_width], self.sigma_thres)
            qys = torch.from_numpy(qys)
            self.pos_thres = torch.normal(
                self.pos_thres, qys).float().to(self.device)
            self.neg_thres = torch.normal(
                self.neg_thres, qys).float().to(self.device)
            ...
            self.neg_thres = torch.normal(
                ...
                size=first_frame_linear.shape,
                dtype=torch.float32).to(self.device)
            print(self.pos_thres)
            print(self.pos_thres.shape)
            '''
            ...
            self.pos_thres = (torch.zeros((self.output_height, self.output_width)) + 0.2).float().to(self.device)
            self.neg_thres = (torch.zeros((self.output_height, self.output_width)) + 0.2).float().to(self.device)
    ...
    def set_dvs_params(self, model: str):
        if model == 'clean':
            ...
            self.shot_noise_rate_hz = 0.001
            #self.refractory_period_s = 0.01
            self.refractory_period_s = 0.0005
    ...
    def _show(self, inp: np.ndarray):
        ...
        cv2.imwrite(str(self.frame_counter) + '.jpg', img*255)
        cv2.waitKey(30)
